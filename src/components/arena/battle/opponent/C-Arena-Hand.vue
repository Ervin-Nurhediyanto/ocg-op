<template>
  <div class="col-12 p-0 h-20vh">
    <div class="row m-0 h-20vh d-flex justify-content-center">
      <div v-for="i in opponent.hand.length" :key="i" class="col-1 p-0 h-15vh d-flex justify-content-center align-content-center border border-2">
        <img
          :class="`h-15vh w-auto cursor-pointer`"
          :src="opponent.hand[i - 1].imageM"
          @click.prevent="onClick(i - 1)"
          @mouseover.prevent="mouseover(opponent.hand[i - 1])"
        />
      </div>
    </div>
  </div>
    ...
    tributeOneUnit (index) {
      ...
      this.$swal.fire({
        ...
        inputValidator: (value) => {
          ...
          if (value === 'u4') { IT = 4 }
          if (value === 'u5') { IT = 5 }
          const UT = this.opponent.field[IT].card // Unit Tribute
          DZ.push(UT)
          BF.splice(IT, 1)
          this.$emit('HAND', { who: 'Opponent', index: index, todo: 'HAND TO FIELD' }) // Call From Hand
          // this.$emit('CFH', { who: 'Opponent', index: index }) // Call From Hand
        ...
        inputValidator: (value) => {
          ...
          this.$swal.fire({
            ...
            inputValidator: (value) => {
              ...
              if (value === 'u4') { IT2 = 4 }
              if (value === 'u5') { IT2 = 5 }
              const UT2 = this.opponent.field[IT2].card // Unit2 Tribut2e
              DZ.push(UT2)
              BF.splice(IT2, 1)
              this.$emit('HAND', { who: 'Opponent', index: index, todo: 'HAND TO FIELD' }) // Call From Hand
              // this.$emit('CFH', { who: 'Opponent', index: index }) // Call From Hand
